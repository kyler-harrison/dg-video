...
def writeMake(srcNames, infoDict, otherDirs, makePath="Makefile"):
	...
	for srcName, srcDeps in infoDict.items():
		...
		if srcDeps["mocDepend"] == True:
			# NOTE the hardcoding, assumes moc files are only in gui dir - lazy man
			mocName = f"gui/moc_{srcName[4:]}"
			mocNames.append(mocName)
			objs.append(f"{mocName}.o")
			mocDepVar = f"{mocName}_DEPENDS = {srcName}.hh $(shell $(GPP) -MM {srcName}.hh | $(GREP))"
...
def main():
	...
	cvDir = "cv"
	guiDir = "gui"
	nestDirs = [cvDir, guiDir]
	files = os.listdir()
	for nestDir in nestDirs: files += [f"{nestDir}/{f}" for f in os.listdir(nestDir)]
	...
	writeMake(srcNames, infoDict, nestDirs)
